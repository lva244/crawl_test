...
while current_header_index < len(header_vina_2):
	...
	while current_stop < max_stop:
		...
		for i in range(current_start, current_stop):
			urls.add(url+header_vina_2[current_header_index]+"-"+str(i).zfill(7)+".html")
		...
		for body in pool.imap(fetch, urls):
			try: 
				...
				print("Error")
		pool.waitall()
	print("Finished " + header_vina_2[current_header_index])
